...
const CurrentWeatherCard = ({ weather, darkMode }) => {
  if (!weather) {
    return (
        <div className={`p-4 sm:p-6 rounded-xl shadow-lg transition-all duration-300 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
  ...
  return (
    <div className={`p-4 sm:p-6 rounded-xl shadow-lg transition-all duration-300 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'}`}>
      ...
      <div className="mt-4 text-center">
        <h1 className="text-5xl sm:text-7xl font-extrabold tracking-tighter">{Math.round(temperature)}°C</h1>
        <p className="text-sm sm:text-base text-gray-400 mt-1">Feels like {Math.round(feelsLike)}°C</p>
      </div>
